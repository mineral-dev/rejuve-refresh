...
export default function Location() {
  ...
  useEffect(()=> {
    if (!errorLoc && dataLoc?.attributes) {
      ...
      db.get('location').then(function(doc) {
        setPost(doc?.data)
  ...
  return (
    <main className="flex-grow">
          ...
          <div className="flex flex-col-reverse lg:grid lg:grid-cols-3 gap-8 mt-12">
            <section>
              <div className="grid gap-y-2 divide-y divide-primary-200">
                {(stores && stores?.length > 0)&& stores.map((item, key) => (
      ...
      {
        post?.Banner &&
        <BannerFooter data={post?.Banner} dbtable="location" />
      }
    </main>
  );
}
  ...
  return (
    <section className="flex space-x-4 p-6">
      ...
      <div>
        ...
        {
          (data?.attributes?.Cta && data?.attributes?.Cta?.length > 0) &&
          data?.attributes?.Cta?.map((item, key) => (
            <Link key={key} target="_blank" href={item?.Link ? item?.Link : '#'} className="btn-primary btn-sm mt-4">
              {item.Caption}
            </Link>
          ))
        }
